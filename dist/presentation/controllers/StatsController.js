            ...
            try {
                ...
                const [currentRes, prevRes] = await Promise.all([
                    supabase_client_1.supabase
                        .from("bookings")
                        .select("id, estado, fecha, hora_inicio, barber_id, service_id, cliente_email, services(nombre, precio)")
                ...
                if (currentRes.error)
                    throw new errors_1.AppError(currentRes.error.message, 500);
                if (prevRes.error)
                    throw new errors_1.AppError(prevRes.error.message, 500);
                ...
                const cancelados = bookings.filter((b) => b.estado === "cancelada").length;
                const tasaCancelacion = totalMes > 0 ? Math.round((cancelados / totalMes) * 100) : 0;
                // ── Ingresos ───────────────────────────────────────────────────────────
                const ingresosMes = activos.reduce((sum, b) => sum + (b.services?.precio ?? 0), 0);
                const ingresosPrev = prevBookings.reduce((sum, b) => sum + (b.services?.precio ?? 0), 0);
                const ingresosVariacion = ingresosPrev > 0
                ...
                const topBarberoId = Object.entries(porBarbero).sort((a, b) => b[1] - a[1])[0]?.[0] ?? null;
                ...
                const topServicio = Object.values(porServicio).sort((a, b) => b.count - a.count)[0] ?? null;
                // ── Hora pico ──────────────────────────────────────────────────────────
                const porHoraMap = {};
                for (let h = 0; h < 24; h++) {
                ...
                const horaPico = distribucionHoras.length > 0
                ...
                if (emailsActivos.length > 0) {
                    const { data: prevClients } = await supabase_client_1.supabase
                        .from("bookings")
                    ...
                    const emailsPrevios = new Set((prevClients ?? []).map((b) => b.cliente_email));
                    for (const email of emailsActivos) {
                        if (emailsPrevios.has(email))
                            clientesRecurrentes++;
                        else
                            clientesNuevos++;
                    }
                ...
                const turnosVariacion = turnosPrev > 0
