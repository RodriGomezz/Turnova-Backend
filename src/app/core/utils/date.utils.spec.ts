import { toDateString, todayString, currentTimeString, formatFechaUY } from './date.utils';
...
describe('date.utils', () => {
  ...
  describe('toDateString()', () => {
    ...
    it('usa hora local, no UTC', () => {
      ...
      const result = toDateString(new Date(2024, 5, 1));
    ...
    it('maneja año de 4 dígitos sin padding', () => {
      expect(toDateString(new Date(2000, 0, 1))).toBe('2000-01-01');
    });
  });
  ...
  describe('todayString()', () => {
    it('devuelve la fecha de hoy en formato YYYY-MM-DD', () => {
      ...
      const expected = toDateString(today);
  ...
  describe('currentTimeString()', () => {
    it('devuelve string en formato HH:MM', () => {
      expect(currentTimeString()).toMatch(/^\d{2}:\d{2}$/);
    });

    it('las horas están en rango 00–23', () => {
      const [hours] = currentTimeString().split(':').map(Number);
      expect(hours).toBeGreaterThanOrEqual(0);
      expect(hours).toBeLessThanOrEqual(23);
    });

    it('los minutos están en rango 00–59', () => {
      const [, minutes] = currentTimeString().split(':').map(Number);
  ...
  describe('formatFechaUY()', () => {
    it('formatea una fecha en español uruguayo', () => {
      ...
      const result = formatFechaUY('2024-06-15');
    ...
    it('incluye el día de la semana por defecto', () => {
      const result = formatFechaUY('2024-01-01');
      // Lunes 1 de enero
    ...
    it('acepta opciones de formato personalizadas', () => {
      const result = formatFechaUY('2024-12-25', { year: 'numeric', month: 'long', day: 'numeric' });
